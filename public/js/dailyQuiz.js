    ...
    if (answerType === "slider" || answerType === "scale") {
      ...
      if (Array.isArray(answerValue)) {
        answerValue.forEach((item) => {
          const key = `${questionId}_${item.replace(/\s+/g, "_").toLowerCase()}`;
          quizDataToSave[key] = 1;
        });

        // Assume all possible options are provided for each checklist question
  ...
  toggles.forEach((toggle) => {
    toggle.addEventListener("change", function () {
      ...
      if (this.name === "answer-currentlyMenstruatingQuestion") {
        const symptomsContainer = document.getElementById(
          "checklist-container-menstrualSymptomsChecklist",
        );
...
function renderChecklistItems(container, questionId, items, question) {
  if (questionId === "menstrualSymptomsChecklist") {
    const menstruatingYes = document.querySelector(
      'input[name="answer-currentlyMenstruatingQuestion"][value="yes"]',
    );
  ...
  items.forEach((item) => {
    ...
    if (
      questionId === "menstrualSymptomsChecklist" ||
      questionId.includes("conditionSymptoms_")
    ) {
    ...
    checkbox.addEventListener("change", function () {
      ...
      if (
        ...
        const severityDiv = document.getElementById(
          `severity-${sanitizedValue}`,
        );
        if (severityDiv) {
    ...
    if (question.answerType === "slider") {
      ...
      allItems.forEach((item) => {
        ...
        if (question.id === "menstrualSymptomsChecklist" && isChecked) {
          const severityInput = document.getElementById(
            `severity-input-${sanitizedItem}`,
          );
          if (severityInput) {
